...
public class Macierz {
    ...
    Macierz add(Macierz macierz2) throws MatrixDimensionException {
        ...
        if (_wysokosc == macierz2._wysokosc && _szerokosc == macierz2._szerokosc) {
            ...
            throw new MatrixDimensionException("Matrix is not good");
        }
    }

    Macierz sub(Macierz macierz2) throws MatrixDimensionException{
        Macierz wynik = new Macierz(_wysokosc,_szerokosc);
        if (_wysokosc == macierz2._wysokosc && _szerokosc == macierz2._szerokosc) {
            for (int i = 0; i < _wysokosc; i++) {
                for (int j = 0; j < _szerokosc; j++) {
    ...
    Macierz mul(Macierz macierz2)throws MatrixDimensionException{
        Macierz wynik = new Macierz(_wysokosc,_szerokosc);
    ...
    public class MatrixDimensionException extends Exception{
        MatrixDimensionException(String error) {
            super (error);
        }
